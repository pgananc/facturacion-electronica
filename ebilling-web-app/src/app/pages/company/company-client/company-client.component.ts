      ...
      .subscribe((data) => {
        if (data != null) {
          ...
          for (let client of this.dataSourceClient.data) {
            client.idTypeEnum = IDENTIFICATION_TYPE.find(
              (idType) => idType.code === client.idType
            )!.value;
            client.clientTypeEnum = CLIENT_TYPE.find(
              (clientType) => clientType.code === client.clientType
            )!.value;
          }
